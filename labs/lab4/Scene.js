class Scene {
    static setGrlobalVariables(){
        ...
        shininessir = document.getElementById('shininess');
    ...
    static changeShininessOnInput(){
        if(addedObjectsMenu.selectedIndex > -1){
            objects[addedObjectsMenu.selectedIndex].changeShininess(+shininessir.value);
        }
    }
    ...
    static openFile(event) {
        ...
        reader.onload = function(){
            ...
            // add to the other objects
            objects.push(new LoadedObject(vertices.length, lines, vec4(r, g, b, 1), 20.0, "Loaded"));
            var option = document.createElement("option");
            option.text = "loaded";
            addedObjectsMenu.add(option, addedObjectsMenu[numObj]);
            addedObjectsMenu.selectedIndex = numObj;
